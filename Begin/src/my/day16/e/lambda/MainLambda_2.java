...
public class MainLambda_2 {
	...
	public static void main(String[] args) {
		...
		InterAreaFuncional_1 obj1 = new InterAreaFuncional_1() {
			@Override
		...
		InterAreaFuncional_1 obj2 = (x, y) -> {return x * y;};
		double areaSize2 = obj2.area(10.5, 5.5);
		
		System.out.println("가로 10.5, 세로 5.5 인 면적은 : " +  areaSize2);
		
		System.out.println("\n~~~~~~~~~~~람다식(Lambda) 표기 방법~~~~~~~~~~~~");
		
		InterAreaFuncional_3 obj = new InterAreaFuncional_3() {
			@Override
		...
		InterAreaFuncional_3 objA =(String state) -> {	System.out.println("웃음소리1 : " + state);};
		objA.smile("깔깔깔~~~~");
		
		//2. 실행 명령문이 1개라면 { }를 생략할 수 있다.
		InterAreaFuncional_3 objB =(String state) -> System.out.println("웃음소리2 : " + state);;
		objB.smile("호호호~~~~");
		
		//3. 파라미터 타입을 생략할 수 있다.
		InterAreaFuncional_3 objC =(state) -> System.out.println("웃음소리3 : " + state);;
		objC.smile("헤헤헤~~~~");
		
		//4. 파라미터의 개수가 1개일 때는 파라미터를 감싸는 괄호()를 생략할 수 있다.
		InterAreaFuncional_3 objD = state -> System.out.println("웃음소리4 : " + state);;
		objD.smile("히히히~~~~");
		
		//5. 파라미터가 없는 람다식은 무조건 () 괄호를 꼭 기재해야한다.
		InterAreaFuncional_4 objE = () -> System.out.println("안녕하세요~~~~");
		objE.write();
		
		//6. 파라미터가 여러 개 일 떄는 무조건 () 괄호를 꼭 기재해야한다.
		InterAreaFuncional_1 objF = (x, y) -> {return x + y;};
		double db1 = objF.area(10.5, 20.5);
		System.out.println(db1);
		
		//7. return 이 있을 경우에는 { }를 생략할 수 없다.
		//InterAreaFuncional_1 objG = (x, y) -> return x + y; //오류
		InterAreaFuncional_1 objG = (x, y) -> {return x - y;}; //오류
		double db2 = objG.area(30.5, 20.5);
		System.out.println(db2);
		
		//8. return 문만 있는 단일 코드인 경우에는 { } 및 return 을 생략할 수 있다.
		InterAreaFuncional_1 objH = (x, y) -> x + y; //오류
