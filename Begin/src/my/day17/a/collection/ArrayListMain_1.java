...
public class ArrayListMain_1 {
	...
	/*
	 	Array와 Collection(구조체)의 차이점은 
	 	1. Array는 크기가 한정되어져 있다. 
	 	   Collection은 크기가 무한정이다.
	 	2. Array는 배열에 선언되어진 동일한 타입의 데이터만 입력간으하지만
	 	   Collection은 입력되어지는 데이터가 객체이기만 하면 어떤 것이든 입력 가능하다.
	 */
...
*/	
		...
		*/
		...
		// Generic(제네릭)
		/*
			  원래 Collection은 Object 이기만하면 모두 입력이 가능하다
			  그런데 입력은 모두 되지만 사용할 떄가 문제가 발생한다.
			  수 많은 데이터중에서 특정 데이터타입을 가지는 데이터를 꺼내오기는 귀찮은 작업을 해야만 한다.
			 그래서 배열처럼 특정한 타입의 데이터만 입력하도록 만든 것이 Generic이다.
		...
		//List<> strlist = new ArrayList<String>(); //오류
		...
		//mbrlist.add(10); // 오류! mbrlist 에는 Generic 으로 선언된 Member 객체만 들어오기 때문
