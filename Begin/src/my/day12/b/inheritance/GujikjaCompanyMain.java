...
public class GujikjaCompanyMain {
	...
	//	System.out.println("co1.isUseCompany() => " + co1.isUseCompany());
		...
		do {
			if(gu == null && co == null) title = "======== >> 메인메뉴 << ========";
			else if(gu == null && co != null)title = "====== >> 메인메뉴[구인회사 " + co.getName()+ "] 로그인 중.. ======";
			else title = "====== >> 메인메뉴[구직자 " + gu.getName()+ "] 로그인 중.. ======";
			...
			switch (smenuNo) {
				...
				case "3": // 구직자 로그인
					...
					else if(co == null && gu != null) System.out.println("현재 구직자로 로그인이 되어있는 상태입니다.");
				...
				case "8": // 내정보 변경하기
					   if(gu != null && co == null)
						   gu = ctrl.updateMyInfo(sc, gu);
					   
					   else if(co != null && gu == null)
						   co = ctrl.updateMyInfo(sc, co);
					   else System.out.println(">> 먼저 로그인 하셔야 합니다. << \n");
		               break;   
